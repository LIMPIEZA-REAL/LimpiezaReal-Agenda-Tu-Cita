...
async function renderHours(date) {
  ...
  try {
    ...
    data.slots.forEach(slot => {
      ...
      if (state.selectedTime === slot) slotBtn.classList.add('selected');
...
function renderClientForm() {
  ...
  form.innerHTML = `
    ...
    <input type="text" id="client-address" placeholder="Tu dirección" required>
  `;
  ...
  confirmBtn.onclick = async () => {
    ...
    try {
      ...
      showToast('Error al agendar, inténtalo nuevamente');
      console.error(e);
    }
